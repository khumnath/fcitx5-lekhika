 ...
 * • Self contained transliterator with header. can be used elsewhere with changing fcitx includes.
 ********************************************************************
...
// Find words with glob?
/* DictionaryManager::findWords(const std::string &prefix, int limit)
{
    std::vector<std::string> results;
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        //  keep the raw UTF-8 bytes, no locale involved
const char *txt = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0));
results.emplace_back(txt);
}
...
DictionaryManager::getAllWords(int limit, int offset) {
  std::vector<std::pair<std::string, int>> results;
  if (!db_) return results;
  sqlite3_stmt *stmt;

  std::string sql_str = "SELECT word, frequency FROM words ORDER BY word ASC";
